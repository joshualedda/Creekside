...
<body>
  ...
  <?php  
    ...
    $query = "SELECT * FROM dishes WHERE status = 'Available' ORDER BY dish_name ASC";
    ...
    if ($result) {
      while ($row = $result->fetch_assoc()) { 
        ...
        <?php if ($role === 'Admin') { ?>
                    <td>
                      <button class="btn btn2 btn-sm" data-bs-toggle="modal" data-bs-target="#updateDish-<?php echo $id ?>"><i class="bi bi-pencil-square"></i> </button>
  ...
  <?php  
    ...
    $username = $_SESSION['username'];
    $query = "SELECT * FROM dishes WHERE status = 'Unavailable' ORDER BY dish_name ASC";
    $result = $con->query($query);
