...
$qry = $conn->query("SELECT * FROM vouchers where id = ".$_GET['id'])->fetch_array();
foreach($qry as $k => $v){
    $$k = $v;
}
// $reference_number = $qry['reference_number']; // Add this line to assign the value to $reference_number
...
//     $branch = array();
//     $tbl_office = $conn->query("SELECT *, office_name as address FROM tbl_office where id in ($to_branch_id,$from_branch_id)");
		...
		<div class="row">
			<div class="col-md-6">
				<div class="callout callout-info">
					...
					<dl>
						<dt>Voucher owner:</dt>
						<dd><?php echo ucwords($voucher_owner) ?></dd>
						<dt>Voucher type:</dt>
						<dd><?php echo ucwords($voucher_type) ?></dd>
						...
						<dd>
							...
							switch ($status) {
								case '1':
									...
									break;
								case '2':
									echo "<span class='badge badge-pill badge-info'> With Suspension</span>";
									break;
								case '3':
									echo "<span class='badge badge-pill badge-primary'> Released</span>";
									break;
								
								
								default:
									echo "<span class='badge badge-pill badge-info'> Incoming</span>";
